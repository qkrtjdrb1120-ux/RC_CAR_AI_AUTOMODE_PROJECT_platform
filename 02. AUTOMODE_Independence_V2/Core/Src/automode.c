...
AutoOutput_t AutoMode_Run(AutoInput_t input)
{
    ...
    switch (s_state)
    {
        ...
        // -------------------------------------------------
            // [판단 1] 너무 가까운가? -> 도망가자!
            if (input.Dist_C <= TARGET_DIST_CM || input.Dist_L <= 6 || input.Dist_R <= 6)
            {
        ...
        // -------------------------------------------------
            ...
            if (input.Dist_L > 7 && input.Dist_R > 7)
            {
